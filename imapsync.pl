...
sub login_imap {
	my (
		...
		$debugimap, $timeout,  $fastio, $ssl,
		$authmech,  $authuser, $reconnectretry
	  )
	  = @_;
	my ($imap);
...
sub plainauth() {
	...
	my $string =
	  sprintf( "%s\x00%s\x00%s", $imap->User, $imap->Authuser,
		$imap->Password );
...
my ( @f_folders, %requested_folder, @t_folders, @t_folders_list,
	%t_folders_list, %subscribed_folder, %t_folders );
