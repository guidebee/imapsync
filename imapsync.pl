...
$imap1 = login_imap(
	$host1,           $port1,      $user1,     $domain1,
	$password1,       $debugimap1, $timeout,   $fastio1,
	$ssl1,            $tls1,       $authmech1, $authuser1,
	$reconnectretry1, $proxyauth1, $uid1,      $split1,
	'Host1',          $ssl1_SSL_version
);

$debugimap2 and print "Host2 connection\n";
$imap2 = login_imap(
	$host2,           $port2,      $user2,     $domain2,
	$password2,       $debugimap2, $timeout,   $fastio2,
	$ssl2,            $tls2,       $authmech2, $authuser2,
	$reconnectretry2, $proxyauth2, $uid2,      $split2,
	'Host2',          $ssl2_SSL_version
);
...
my (
	...
	%h2_folders_all_UPPER,     @h2_folders_from_1_wanted,
	%h2_folders_from_1_wanted, %h2_folders_from_1_several,
...
if ( $fixInboxINBOX
	and
	( my $reg = fix_Inbox_INBOX_mapping( \%h1_folders_all, \%h2_folders_all ) )
  )
{
...
if ($foldersizes) {
	( $h1_nb_msg_at_start, $h1_bytes_start ) =
	  foldersizes( "Host1", $imap1, $search1, @h1_folders_wanted );
	( $h2_nb_msg_start, $h2_bytes_start ) =
	  foldersizes( "Host2", $imap2, $search2, @h2_folders_from_1_wanted );
	$fast or sleep(2);
}
...
FOLDER: foreach my $h1_fold (@h1_folders_wanted) {
	...
	if (
		(
			( $subscribe and exists $h1_subscribed_folder{$h1_fold} )
			or $subscribe_all
		)
	...
	my @h1_msgs =
	  select_msgs( $imap1, $h1_msgs_all_hash_ref, $search1, $h1_fold );
	last FOLDER if $imap1->IsUnconnected();
	...
	my @h2_msgs =
	  select_msgs( $imap2, $h2_msgs_all_hash_ref, $search2, $h2_fold );
	last FOLDER if $imap2->IsUnconnected();
	...
	my $cache_dir =
	  cache_folder( $cache_base, "$host1/$user1/$host2/$user2", $h1_fold,
		$h2_fold );
	...
	if ($usecache) {
		...
		( $cache_1_2_ref, $cache_2_1_ref ) =
		  get_cache( $cache_dir, \@h1_msgs, \@h2_msgs, $h1_msgs_all_hash_ref,
	...
	$h1_heads_ref = $imap1->parse_headers( [@h1_msgs_not_in_cache], @useheader )
	  if (@h1_msgs_not_in_cache);
	$debug
	  and print "Host1 parsing headers of folder [$h1_fold] took ", timenext(),
	  " s\n";

	@$h1_fir_ref{@h1_msgs} = (undef);

	$debug
	  and print "Host1 getting flags idate and sizes of folder [$h1_fold]\n";
	if ($abletosearch) {
		$h1_fir_ref =
		  $imap1->fetch_hash( \@h1_msgs, "FLAGS", "INTERNALDATE", "RFC822.SIZE",
			$h1_fir_ref )
		  if (@h1_msgs);
	}
	else {
		my $uidnext = $imap1->uidnext($h1_fold) || $uidnext_default;
		$h1_fir_ref = $imap1->fetch_hash(
			"1:$uidnext", "FLAGS", "INTERNALDATE", "RFC822.SIZE",
			$h1_fir_ref
		) if (@h1_msgs);
	}
	$debug
	...
	foreach my $m (@h1_msgs_not_in_cache) {
		my $rc =
		  parse_header_msg( $imap1, $m, $h1_heads_ref, $h1_fir_ref, 'Host1',
			\%h1_hash );
		if ( !defined($rc) ) {
...
"Host1 $h1_fold/$m size $h1_size ignored (no wanted headers so we ignore this message. To solve this: use --addheader)\n";
	...
	$debug
	  and print "Host1 whole time parsing headers took ", timenext(), " s\n";

	$debug and print "Host2 parsing headers of folder [$h2_fold]\n";

	my ( $h2_heads_ref, $h2_fir_ref ) = ( {}, {} );
	$h2_heads_ref = $imap2->parse_headers( [@h2_msgs_not_in_cache], @useheader )
	  if (@h2_msgs_not_in_cache);
	$debug
	  and print "Host2 parsing headers of folder [$h2_fold] took ", timenext(),
	  " s\n";

	$debug
	  and print "Host2 getting flags idate and sizes of folder [$h2_fold]\n";
	@$h2_fir_ref{@h2_msgs} =
	  ();    # fetch_hash can select by uid with last arg as ref

	if ($abletosearch) {
		$h2_fir_ref =
		  $imap2->fetch_hash( \@h2_msgs, "FLAGS", "INTERNALDATE", "RFC822.SIZE",
			$h2_fir_ref )
		  if (@h2_msgs);
	}
	else {
		my $uidnext = $imap2->uidnext($h2_fold) || $uidnext_default;
		$h2_fir_ref = $imap2->fetch_hash(
			"1:$uidnext", "FLAGS", "INTERNALDATE", "RFC822.SIZE",
			$h2_fir_ref
		) if (@h2_msgs);
	}
	...
	foreach my $m (@h2_msgs_not_in_cache) {
		my $rc =
		  parse_header_msg( $imap2, $m, $h2_heads_ref, $h2_fir_ref, 'Host2',
			\%h2_hash );
...
"Host2 folder $h2_fold selected: $h2_msgs_nb messages,  duplicates: $h2_msgs_duplicate_nb\n"
	...
	$debug
	  and print "Host2 whole time parsing headers took ", timenext(), " s\n";
	...
	if ( $delete2duplicates
		and not exists( $h2_folders_from_1_several{$h2_fold} ) )
	{
		my @h2_expunge;
...
"Host1 msg $h1_fold/$h1_msg marked deleted $expunge_message $dry_message\n";
  ...
  MESS_IN_CACHE: foreach my $h1_msg (@h1_msgs_in_cache) {
		...
		sync_flags_fir( $h1_fold, $h1_msg, $h2_fold, $h2_msg, $permanentflags2,
			$h1_fir_ref, $h2_fir_ref );
...
sub sync_flags_fir {
	my ( $h1_fold, $h1_msg, $h2_fold, $h2_msg, $permanentflags2, $h1_fir_ref,
		$h2_fir_ref )
	  = @_;
...
sub sync_flags_after_copy {
	my ( $h1_fold, $h1_msg, $h1_flags, $h2_fold, $h2_msg, $permanentflags2 ) =
	  @_;
...
sub sync_flags {
	my ( $h1_fold, $h1_msg, $h1_flags, $h2_fold, $h2_msg, $h2_flags,
		$permanentflags2 )
	  = @_;
...
sub modules_VERSION {
	...
	eval { require IO::Socket::INET; $v = $IO::Socket::INET::VERSION }
	  or $v = "?";
	push( @list_version, module_version_str( 'IO::Socket::INET', $v ) );

	eval { require IO::Socket::SSL; $v = $IO::Socket::SSL::VERSION }
	  or $v = "?";
	...
	eval { require Digest::HMAC_MD5; $v = $Digest::HMAC_MD5::VERSION }
	  or $v = "?";
	push( @list_version, module_version_str( 'Digest::HMAC_MD5', $v ) );

	eval { require Digest::HMAC_SHA1; $v = $Digest::HMAC_SHA1::VERSION }
	  or $v = "?";
	...
	eval { require Unicode::String; $v = $Unicode::String::VERSION }
	  or $v = "?";
...
sub command_line_nopassword {
	...
	while (@argv) {
		...
		if ( $arg =~ m/-password[12]/x ) {
			...
			push( @argv_nopassword, $arg, "MASKED" )
			  ;                    # option name and fake value
		}
		else {
...
sub tests_command_line_nopassword {
	...
	ok(
		...
		'command_line_nopassword --password1'
	);
	ok(
		'--blabla --password1 MASKED --blibli' eq
		  command_line_nopassword(qw{ --blabla --password1 secret1 --blibli }),
		'command_line_nopassword --password1 --blibli'
	);
	$showpasswords = 1;
	ok( '' eq command_line_nopassword(), 'command_line_nopassword void' );
	ok( '--blabla' eq command_line_nopassword('--blabla'),
	...
	ok(
		'--blabla --password1 secret1 --blibli' eq
		  command_line_nopassword(qw{ --blabla --password1 secret1 --blibli }),
		'command_line_nopassword --password1 --blibli'
	);
	return ();
...
sub relogin1 {
	$imap1 = relogin_imap(
		$imap1,     $host1,           $port1,      $user1,
		$domain1,   $password1,       $debugimap1, $timeout,
		$fastio1,   $ssl1,            $tls1,       $authmech1,
		$authuser1, $reconnectretry1, $proxyauth1, $uid1,
		$split1
	);
...
sub relogin2 {
	$imap2 = relogin_imap(
		$imap2,     $host2,           $port2,      $user2,
		$domain2,   $password2,       $debugimap2, $timeout,
		$fastio2,   $ssl2,            $tls2,       $authmech2,
		$authuser2, $reconnectretry2, $proxyauth2, $uid2,
		$split2
	);
...
sub relogin_imap {
	my (
		$imap,     $host,           $port,        $user,
		$domain,   $password,       $mydebugimap, $mytimeout,
		$fastio,   $ssl,            $tls,         $authmech,
		$authuser, $reconnectretry, $proxyauth,   $uid,
		$split
	) = @_;
	...
	$imap->logout();
	$imap = login_imap(
		$host,           $port,        $user,      $domain,
		$password,       $mydebugimap, $mytimeout, $fastio,
		$ssl,            $tls,         $authmech,  $authuser,
		$reconnectretry, $proxyauth,   $uid,       $split
	);
...
sub authenticate_imap {
	...
	if ($proxyauth) {
		...
		$imap->Authmechanism($authmech)
		  unless ( $authmech eq 'LOGIN' or $authmech eq 'PREAUTH' );
		$imap->User($user);
	}

	$imap->Authcallback( \&xoauth ) if ( 'XOAUTH' eq $authmech );
	$imap->Authcallback( \&plainauth )
	  if ( ( 'PLAIN' eq $authmech ) or ( 'EXTERNAL' eq $authmech ) );
...
sub set_tls {
	my ($imap) = @_;
	my $tlsargs = [ SSL_verify_mode => 'SSL_VERIFY_NONE', ];
...
sub plainauth {
	...
	my $string = sprintf( "%s\x00%s\x00%s", $imap->User, $imap->Authuser,
		$imap->Password );
...
sub xoauth {
	...
	my $URLparm = sprintf( "xoauth_requestor_id=%s",
		URI::Escape::uri_escape( $imap->User ) );
	...
	my $digest =
	  hmac_sha1( $base, URI::Escape::uri_escape( $imap->Password ) . "&" );

	# The parameters signed become a parameter and...
	$hash{'oauth_signature'} =
	  URI::Escape::uri_escape( substr( encode_base64($digest), 0, -1 ) );
...
sub banner_imapsync {
	...
	my $banner_imapsync = join( "",
		'$RCSfile: imapsync,v $ ',       '$Revision: 1.607 $ ',
		'$Date: 2014/11/14 16:25:06 $ ', "\n",
		localhost_info(),                "\n",
		"Command line used:\n",          "$0 ",
		command_line_nopassword(@argv),  "\n",
	);
...
sub fix_Inbox_INBOX_mapping {
	...
	my $regex = '';
  SWITCH: {
		if ( exists( $h1_all->{INBOX} ) and exists( $h2_all->{INBOX} ) ) {
			$regex = '';
			last SWITCH;
		}
		if ( exists( $h1_all->{Inbox} ) and exists( $h2_all->{Inbox} ) ) {
...
sub tests_fix_Inbox_INBOX_mapping {
	...
	$h2_all = { 'INBOX' => '' };
	ok(
		'' eq fix_Inbox_INBOX_mapping( $h1_all, $h2_all ),
		'fix_Inbox_INBOX_mapping: INBOX INBOX'
	);
	...
	$h1_all = { 'INBOX' => '' };
	$h2_all = { 'Inbox' => '' };
	ok( 's/^INBOX$/Inbox/x' eq fix_Inbox_INBOX_mapping( $h1_all, $h2_all ),
		'fix_Inbox_INBOX_mapping: INBOX Inbox' );
	...
	ok(
		'' eq fix_Inbox_INBOX_mapping( $h1_all, $h2_all ),
		'fix_Inbox_INBOX_mapping: INBOX rrrrrr'
	);
	...
	ok(
		'' eq fix_Inbox_INBOX_mapping( $h1_all, $h2_all ),
		'fix_Inbox_INBOX_mapping: rrrrr Inbox'
	);
...
sub tests_jux_utf8 {
	...
	ok(
		'[&ZTZO9nux-] = [收件箱]' eq jux_utf8('&ZTZO9nux-'),
		'jux_utf8: &ZTZO9nux- => [&ZTZO9nux-] = [收件箱]'
	);
	ok(
		'[&ANY-] = [Ö]' eq jux_utf8('&ANY-'),
		'jux_utf8: &ANY- => [&ANY-] = [Ö]'
	);
...
sub tests_folder_routines {
	ok( !is_requested_folder('folder_foo'),
		'is_requested_folder folder_foo 1' );
	ok(
		add_to_requested_folders('folder_foo'),
		'add_to_requested_folders folder_foo'
	);
	ok( is_requested_folder('folder_foo'), 'is_requested_folder folder_foo 2' );
	ok(
		!is_requested_folder('folder_NO_EXIST'),
		'is_requested_folder folder_NO_EXIST'
	);
	ok( !remove_from_requested_folders('folder_foo'), 'removed folder_foo' );
	ok( !is_requested_folder('folder_foo'),
		'is_requested_folder folder_foo 3' );
	my @f;
	ok( @f = add_to_requested_folders( 'folder_bar', 'folder_toto' ),
		"add result: @f" );
	...
	ok(
		!remove_from_requested_folders('folder_bar'),
		"remove_from_requested_folders: empty"
	);

	ok( 0 == compare_lists( [ sort_requested_folders() ], [] ),
		'sort_requested_folders: all empty' );
	ok( add_to_requested_folders('M_55'), 'add_to_requested_folders M_55' );
	ok( 0 == compare_lists( [ sort_requested_folders() ], ['M_55'] ),
		'sort_requested_folders: middle' );
	@folderfirst = ('Z_11');
	ok(
		0 == compare_lists( [ sort_requested_folders() ], [ 'Z_11', 'M_55' ] ),
		'sort_requested_folders: first+middle'
	);
	@folderlast = ('A_99');
	ok(
		0 == compare_lists(
		...
		'sort_requested_folders: first+middle+last 1'
	);

	ok(
		add_to_requested_folders( 'M_55', 'M_44', ),
		'add_to_requested_folders M_55 M_44'
	);
	ok(
		0 == compare_lists(
		...
		'sort_requested_folders: first+middle+last 2'
	);
	...
	ok(
		0 == compare_lists(
			[ sort_requested_folders() ],
			[ 'Z_22', 'Z_11', 'M_44', 'M_55', 'A_99', 'A_88' ]
		),
		'sort_requested_folders: first+middle+last 3'
	);
...
sub compare_lists {
	...
	return (0)
	  if ( ( not defined($list_1_ref) ) and not defined($list_2_ref) )
	  ;    # end if no list
...
sub tests_compare_lists {
	...
	ok( +1 == compare_lists( [ 1, 1 ], [1] ), "compare_lists, [1, 1] > [1]" );
	ok(
		0 == compare_lists( [ 1 .. 20_000 ], [ 1 .. 20_000 ] ),
	...
	ok( 0 == compare_lists( ["ab"], ["ab"] ),
		'compare_lists, ["ab"] = ["ab"]' );
	ok( +1 == compare_lists( ["b"], ["a"] ),  'compare_lists, ["b"] > ["a"]' );
	ok( -1 == compare_lists( ["a"], ["aa"] ), 'compare_lists, ["a"] < ["aa"]' );
	ok(
		-1 == compare_lists( ["a"], [ "a", "a" ] ),
		'compare_lists, ["a"] < ["a", "a"]'
	);
	ok( 0 == compare_lists( [ split( " ", "a b" ) ], [ "a", "b" ] ),
		'compare_lists, split' );
	ok( 0 == compare_lists( [ sort split( " ", "b a" ) ], [ "a", "b" ] ),
		'compare_lists, sort split' );
	return ();
}
...
sub tests_separator_invert {
	...
	ok(
		not( defined( separator_invert('') ) ),
		'separator_invert: not enough args'
	);
	ok(
		not( defined( separator_invert( '', '' ) ) ),
		'separator_invert: not enough args'
	...
	ok(
		'' eq separator_invert( '', '', '' ),
		'separator_invert: 3 empty strings'
	);
	ok( 'lalala' eq separator_invert( 'lalala', '', '' ),
		'separator_invert: empty separator' );
	ok( 'lalala' eq separator_invert( 'lalala', '/', '/' ),
		'separator_invert: same separator /' );
	ok( 'lal/ala' eq separator_invert( 'lal/ala', '/', '/' ),
		'separator_invert: same separator / 2' );
	ok( 'lal.ala' eq separator_invert( 'lal/ala', '/', '.' ),
		'separator_invert: separators /.' );
	ok( 'lal/ala' eq separator_invert( 'lal.ala', '.', '/' ),
		'separator_invert: separators ./' );
	ok( 'la.l/ala' eq separator_invert( 'la/l.ala', '.', '/' ),
		'separator_invert: separators ./' );

	ok( 'l/al.ala' eq separator_invert( 'l.al/ala', '/', '.' ),
		'separator_invert: separators /.' );
	$fixslash2 = 1;
	ok( 'l_al.ala' eq separator_invert( 'l.al/ala', '/', '.' ),
		'separator_invert: separators /.' );
...
sub separator_invert {
	...
	return (undef)
	  if ( not defined($h1_fold)
		or not defined($h1_separator)
		or not defined($h2_separator) );
	...
	$h2_fold =~ s,/,_,xg
	  if ( $fixslash2 and '/' ne $h2_separator and '/' eq $h1_separator );
	return ($h2_fold);
}
...
EOS
	...
	ok( 'spam.spam' eq imap2_folder_name('spam/spam'),
		'imap2_folder_name: spam/spam' );
	ok( 'spam/spam' eq imap2_folder_name('spam.spam'),
		'imap2_folder_name: spam.spam' );
	ok( 'spam.spam/spam' eq imap2_folder_name('spam/spam.spam'),
		'imap2_folder_name: spam/spam.spam' );
	ok( 's pam.spam/sp  am' eq imap2_folder_name('s pam/spam.sp  am'),
		'imap2_folder_name: s pam/spam.sp  am' );
	@regextrans2 = ('s,/,X,g');
	ok( '' eq imap2_folder_name(''),
		'imap2_folder_name: empty string [s,/,X,g]' );
	ok(
		'blabla' eq imap2_folder_name('blabla'),
		'imap2_folder_name: blabla [s,/,X,g]'
	);
	ok(
		'spam.spam' eq imap2_folder_name('spam/spam'),
		'imap2_folder_name: spam/spam [s,/,X,g]'
	);
	ok(
		'spamXspam' eq imap2_folder_name('spam.spam'),
		'imap2_folder_name: spam.spam [s,/,X,g]'
	);
	ok(
		'spam.spamXspam' eq imap2_folder_name('spam/spam.spam'),
		'imap2_folder_name: spam/spam.spam [s,/,X,g]'
	);

	@regextrans2 = ('s, ,_,g');
	ok(
		'blabla' eq imap2_folder_name('blabla'),
		'imap2_folder_name: blabla [s, ,_,g]'
	);
	ok(
		'bla_bla' eq imap2_folder_name('bla bla'),
		'imap2_folder_name: blabla [s, ,_,g]'
	);
	...
	ok(
		'BLABLA' eq imap2_folder_name('blabla'),
		'imap2_folder_name: blabla [s,\U(.*)\E,$1,]'
	);
	...
	ok(
		'spam.spam' eq imap2_folder_name('spam/spam'),
		'imap2_folder_name: spam/spam -> spam.spam'
	);
	ok(
		'spam_spam' eq imap2_folder_name('spam.spam'),
		'imap2_folder_name: spam.spam -> spam_spam'
	);
	ok(
		'spam.spam_spam' eq imap2_folder_name('spam/spam.spam'),
		'imap2_folder_name: spam/spam.spam -> spam.spam_spam'
	);
	ok(
		's pam.spam_spa  m' eq imap2_folder_name('s pam/spam.spa  m'),
		'imap2_folder_name: s pam/spam.spa m -> s pam.spam_spa  m'
	);
	...
	ok(
		'spam.spam/spam' eq imap2_folder_name('spam/spam.spam'),
		'imap2_folder_name: spam/spam.spam -> spam.spam/spam'
	);
	...
	ok( 'BLABLA' eq imap2_folder_name('blabla'), 'imap2_folder_name: blabla' );
	ok(
		'TEST/TEST/TEST/TEST' eq imap2_folder_name('INBOX.TEST.test.Test.tesT'),
		'imap2_folder_name: INBOX.TEST.test.Test.tesT'
	);
	@regextrans2 = ('s,(.*),\L$1,');
	ok(
		'test/test/test/test' eq imap2_folder_name('INBOX.TEST.test.Test.tesT'),
...
sub tests_decompose_regex {
	...
	ok( 0 == compare_lists( [ '', '' ], [ decompose_regex('') ] ),
		'decompose_regex empty string' );
	ok(
		0 ==
		  compare_lists( [ '.*', 'lala' ], [ decompose_regex('s/.*/lala/') ] ),
		'decompose_regex s/.*/lala/'
	);
	return ();
}
...
sub foldersizes {
	...
	foreach my $folder (@folders) {
		...
		if ( $nb_msgs > 0 and @msgs ) {
			if ($abletosearch) {
				$imap->fetch_hash( \@msgs, "RFC822.SIZE", $hash_ref )
				  or die_clean("$@");
			}
			else {
				my $uidnext = $imap->uidnext($folder) || $uidnext_default;
				$imap->fetch_hash( "1:$uidnext", "RFC822.SIZE", $hash_ref )
				  or die_clean("$@");
			}
			for ( keys %$hash_ref ) {
...
sub tests_flags_regex {
	...
	ok( '\Seen NonJunk $Spam' eq flags_regex('\Seen NonJunk $Spam'),
		'flags_regex, nothing to do' );
	...
	ok(
		'\Seen  $Spam' eq flags_regex('\Seen NonJunk $Spam'),
		"flags_regex, remove NonJunk: 's/NonJunk//g'"
	);
	@regexflag = ('s/\$Spam//g');
	ok(
		'\Seen NonJunk ' eq flags_regex('\Seen NonJunk $Spam'),
		'flags_regex, remove $Spam: ' . "'s/\$Spam//g'"
	);

	@regexflag = ('s/\\\\Seen//g');

	ok(
		' NonJunk $Spam' eq flags_regex('\Seen NonJunk $Spam'),
		'flags_regex, remove \Seen: ' . "'s/\\\\\\\\Seen//g'"
	);

	@regexflag = ('s/(\s|^)[^\\\\]\w+//g');
	ok(
		'\Seen \Middle \End' eq flags_regex('\Seen NonJunk \Middle $Spam \End'),
		'flags_regex, only \word ['
		  . flags_regex( '\Seen NonJunk \Middle $Spam \End' . ']' )
	);
	ok(
		' \Seen \Middle \End1' eq
		  flags_regex('Begin \Seen NonJunk \Middle $Spam \End1 End'),
		'flags_regex, only \word ['
		  . flags_regex( 'Begin \Seen NonJunk \Middle $Spam \End1 End' . ']' )
	);

	@regexflag = ('s/.*?(Keep1|Keep2|Keep3)/$1 /g');
	ok( 'Keep1 Keep2  ReB' eq flags_regex('ReA Keep1 REM Keep2 ReB'),
		"Keep only regex" );

	#ok('Keep1 Keep2' eq flags_regex('Keep1 Keep2 Remove1'), "Keep only regex");
	ok( 'Keep1 Keep2 ' eq flags_regex('REM REM Keep1 Keep2'),
		"Keep only regex" );
	ok( 'Keep1 Keep2 ' eq flags_regex('Keep1 REM REM Keep2'),
		"Keep only regex" );
	ok( 'Keep1 Keep2 ' eq flags_regex('REM Keep1 REM REM  Keep2'),
		"Keep only regex" );
	...
	ok( 'Keep1 Keep2 ' eq flags_regex('Keep1 Keep2 REM REM  REM'),
		"Keep only regex" );
	...
	@regexflag = (
		's/.*?(Keep1|Keep2|Keep3)/$1 /g',
		's/(Keep1|Keep2|Keep3) (?!(Keep1|Keep2|Keep3)).*/$1 /g'
	);
	ok( 'Keep1 Keep2 ' eq flags_regex('REM Keep1 REM Keep2 REM'),
		"Keep only regex" );
	ok( 'Keep1 Keep2 ' eq flags_regex('Keep1 REM Keep2 REM'),
		"Keep only regex" );
	ok( 'Keep1 Keep2 ' eq flags_regex('REM Keep1 Keep2 REM'),
		"Keep only regex" );
	ok( 'Keep1 Keep2 ' eq flags_regex('REM Keep1 REM Keep2'),
		"Keep only regex" );
	ok(
		'Keep1 Keep2 Keep3 ' eq
		  flags_regex('REM Keep1 REM Keep2 REM REM Keep3 REM'),
		"Keep only regex"
	);
	ok( 'Keep1 ' eq flags_regex('REM  REM Keep1 REM REM REM '),
		"Keep only regex" );
	ok( 'Keep1 Keep3 ' eq flags_regex('RE1 Keep1 RE2 Keep3 RE3 RE4 RE5 '),
		"Keep only regex" );

	@regexflag = ('s/(.*)/$1 jrdH8u/');
	ok(
		'REM  REM  REM REM REM jrdH8u' eq flags_regex('REM  REM  REM REM REM'),
		"Keep only regex 's/(.*)/\$1 jrdH8u/'"
	);
	@regexflag = ('s/jrdH8u *//');
	ok(
		'REM  REM  REM REM REM ' eq flags_regex('REM  REM  REM REM REM jrdH8u'),
		"Keep only regex s/jrdH8u *//"
	);
	...
	ok( 'Keep1 Keep2 ' eq flags_regex('REM Keep1 REM Keep2 REM'),
		"Keep only regex 'REM Keep1 REM Keep2 REM'" );
	ok( 'Keep1 Keep2 ' eq flags_regex('Keep1 REM Keep2 REM'),
		"Keep only regex" );
	ok( 'Keep1 Keep2 ' eq flags_regex('REM Keep1 Keep2 REM'),
		"Keep only regex" );
...
's/(\\\\Seen|\\\\Answered|\\\\Flagged|\\\\Deleted|\\\\Draft|jrdH8u) (?!(\\\\Seen|\\\\Answered|\\\\Flagged|\\\\Deleted|\\\\Draft|jrdH8u)).*/$1 /g',
	...
	ok(
		'\\Deleted \\Answered ' eq
		  flags_regex('Blabla $Junk \\Deleted machin \\Answered truc'),
		"Keep only regex: Exchange case"
	);
	ok( '' eq flags_regex(''), "Keep only regex: Exchange case, null string" );
	ok(
		'' eq flags_regex('Blabla $Junk  machin  truc'),
		"Keep only regex: Exchange case, no accepted flags "
	);
	ok(
		'\\Deleted \\Answered \\Draft \\Flagged ' eq
...
's/.*?(?:(\\\\(?:Answered|Flagged|Deleted|Seen|Draft)\s?)|$)/defined($1)?$1:q()/eg'
	...
	ok( '' eq flags_regex(''),
		"Keep only regex: Exchange case, null string (Phil)" );
...
sub tests_permanentflags {
	...
	ok(
		'\Draft \Answered' eq
		  permanentflags(' * OK [PERMANENTFLAGS (\Draft \Answered)] Limited'),
		'permanentflags \Draft \Answered'
	);
	ok(
		'\Draft \Answered' eq permanentflags(
			'Blabla', ' * OK [PERMANENTFLAGS (\Draft \Answered)] Limited',
			'Blabla'
		),
		'permanentflags \Draft \Answered'
	);
...
sub tests_flags_filter {
	...
	ok( '\Seen' eq flags_filter( '\Seen', '\Draft \Seen \Answered' ),
		'flags_filter ' );
	...
	ok(
		'\Seen \Draft' eq
		  flags_filter( '\Seen \Draft', '\Draft \Seen \Answered' ),
		'flags_filter '
	);
	ok(
		'\Seen \Draft' eq
...
sub flagsCase {
	...
	my @flags = split( /\s+/x, $flags );
	my %rfc_flags =
	  map { $_ => 1 } split( ' ', '\Answered \Flagged \Deleted \Seen \Draft' );
	my @flags_out = map {
		exists $rfc_flags{ ucsecond( lc($_) ) } ? ucsecond( lc($_) ) : $_
	} @flags;
...
sub tests_flagsCase {
	...
	ok(
		'\Seen \Draft' eq flagsCase('\SEEN \DRAFT'),
		'flagsCase: \SEEN \DRAFT -> \Seen \Draft'
	);
	ok(
		'\Draft \Seen' eq flagsCase('\DRAFT \SEEN'),
		'flagsCase: \DRAFT \SEEN -> \Draft \Seen'
	);

	ok(
		'\Draft LALA \Seen' eq flagsCase('\DRAFT  LALA \SEEN'),
		'flagsCase: \DRAFT  LALA \SEEN -> \Draft LALA \Seen'
	);
	ok(
		'\Draft lala \Seen' eq flagsCase('\DRAFT  lala \SEEN'),
		'flagsCase: \DRAFT  lala \SEEN -> \Draft lala \Seen'
...
sub ucsecond {
	...
	$output = substr( $string, 0, 1 ) . uc( substr( $string, 1, 1 ) )
	  if ( 2 == length($string) );
	$output =
	    substr( $string, 0, 1 )
...
sub select_msgs {
	...
	if ($abletosearch) {
		@msgs = select_msgs_by_search( $imap, $msgs_all_hash_ref, $search_cmd,
			$folder );
	}
	else {
		@msgs = select_msgs_by_fetch( $imap, $msgs_all_hash_ref, $search_cmd,
			$folder );
	}
...
sub select_msgs_by_search {
	...
	if (
		( defined($msgs_all_hash_ref) and $usecache )
		or (    not defined($maxage)
			and not defined($minage)
			and not defined($search_cmd) )
	  )
	{
...
"Warning: strange to see --search with --noabletosearch, an error can happen\n";
	...
	if ( defined($maxage) ) {
		$maxage_epoch = $timestart_int - 86400 * $maxage;
	}
	if ( defined($minage) ) {
		$minage_epoch = $timestart_int - 86400 * $minage;
	}
...
sub tests_msgs_from_maxmin {
	...
	ok( 0 == compare_lists( [ '1', '2' ], \@msgs ),
		'msgs_from_maxmin: maxage++' );
	$minage = 100;
	@msgs = msgs_from_maxmin( [ '1', '2' ], [ '2', '3' ] );
	ok(
		0 == compare_lists( ['2'], \@msgs ),
		'msgs_from_maxmin:  -maxage++minage-'
	);
	$minage = 300;
	@msgs = msgs_from_maxmin( [ '1', '2' ], [ '2', '3' ] );
	ok(
		0 == compare_lists( [ '1', '2', '3' ], \@msgs ),
		'msgs_from_maxmin:  ++maxage-minage++'
	);
	$maxage = undef;
	@msgs = msgs_from_maxmin( [ '1', '2' ], [ '2', '3' ] );
	ok( 0 == compare_lists( [ '2', '3' ], \@msgs ),
		'msgs_from_maxmin:  ++minage-' );
}
...
"- msg $h1_fold/$h1_msg S[$h1_size] F[$h1_flags] I[$h1_idate] has RFC822.SIZE null!\n"
	...
	( $string, $string_len ) =
	  message_for_host2( $h1_msg, $h1_fold, $h1_size, $h1_flags, $h1_idate,
		$h1_fir_ref );
...
"- msg $h1_fold/$h1_msg skipped S[$h1_size] F[$h1_flags] I[$h1_idate] "
		...
		if ( defined($minmaxlinelength)
			and ( $maxlinelength_string <= $minmaxlinelength ) )
		{
			my $subject = subject($string);
			$debugdev
			  and print
...
"Host1 flags filt msg $h1_fold/$h1_msg date [$h1_date] flags [$h1_flags] size [$h1_size]\n";
	...
	my $new_id = append_message_on_host2(
		$string,  $h1_fold,  $h1_msg,  $string_len, $h2_fold,
		$h1_size, $h1_flags, $h1_date, $cache_dir
	);

	if ( $new_id and $syncflagsaftercopy ) {
		sync_flags_after_copy( $h1_fold, $h1_msg, $h1_flags, $h2_fold, $new_id,
			$permanentflags2 );
	}
...
sub flags_for_host2 {
	...
	$h1_flags = flags_filter( $h1_flags, $permanentflags2 )
	  if ( $permanentflags2 and $filterflags );
...
sub append_message_on_host2 {
	...
	if ( !$dry ) {
		...
		$new_id =
		  $imap2->append_string( $h2_fold, $string, $h1_flags, $h1_date );
		if ( !$new_id ) {
			my $subject = subject($string);
...
"- msg $h1_fold/$h1_msg {$string_len} couldn't append  (Subject:[$subject]) to folder $h2_fold: $error\n";
			...
			my $rate =
			  bytes_display_string( $total_bytes_transferred / $time_spent );
			my $eta = eta(
				$time_spent,         $h1_nb_msg_processed,
				$h1_nb_msg_at_start, $nb_msg_transferred
			);
...
sub sleep_if_needed {
	...
	my $sleep_max_messages =
	  sleep_max_messages( $nb_msg_transferred, $time_spent,
		$maxmessagespersecond );
	my $sleep_max_bytes =
	  sleep_max_bytes( $total_bytes_transferred, $time_spent,
		$maxbytespersecond );
...
sub sleep_max_messages {
	...
	if ( ( not defined($maxmessagespersecond) ) or $maxmessagespersecond <= 0 )
	{
		return (0);
	}
...
sub tests_sleep_max_messages {
	ok( 0 == sleep_max_messages( 4, 2, undef ),
		'sleep_max_messages: maxmessagespersecond = undef' );
	ok( 0 == sleep_max_messages( 4, 2, 0 ),
		'sleep_max_messages: maxmessagespersecond = 0' );
	ok( 0 == sleep_max_messages( 4, 2, -1 ),
		'sleep_max_messages: maxmessagespersecond = -1' );
	ok( 0 == sleep_max_messages( 4, 2, 2 ),
		'sleep_max_messages: maxmessagespersecond = 2 max reached' );
	ok( 2 == sleep_max_messages( 8, 2, 2 ),
		'sleep_max_messages: maxmessagespersecond = 2 max over' );
	ok( 0 == sleep_max_messages( 2, 2, 2 ),
		'sleep_max_messages: maxmessagespersecond = 2 max not reached' );
	return ();
}
...
sub sleep_max_bytes {
	...
	if ( ( not defined($maxbytespersecond) ) or $maxbytespersecond <= 0 ) {
		return (0);
	}
...
sub tests_sleep_max_bytes {
	ok(
		0 == sleep_max_bytes( 4000, 2, undef ),
		'sleep_max_bytes: maxbytespersecond = undef'
	);
	ok(
		0 == sleep_max_bytes( 4000, 2, 0 ),
		'sleep_max_bytes: maxbytespersecond = 0'
	);
	ok(
		0 == sleep_max_bytes( 4000, 2, -1 ),
		'sleep_max_bytes: maxbytespersecond = -1'
	);
	ok( 0 == sleep_max_bytes( 4000, 2, 2000 ),
		'sleep_max_bytes: maxbytespersecond = 2 max reached' );
	ok( 2 == sleep_max_bytes( 8000, 2, 2000 ),
		'sleep_max_bytes: maxbytespersecond = 2 max over' );
	ok( 0 == sleep_max_bytes( 2000, 2, 2000 ),
		'sleep_max_bytes: maxbytespersecond = 2 max not reached' );
	return ();
}
...
sub eta {
	my ( $my_time_spent, $h1_nb_processed, $h1_nb_msg_start, $nb_transferred ) =
	  @_;
	return ('') if not $foldersizes;

	my $time_remaining =
	  time_remaining( $my_time_spent, $h1_nb_processed, $h1_nb_msg_start,
		$nb_transferred );
	my $nb_msg_remaining = $h1_nb_msg_start - $h1_nb_processed;
	my $eta_date         = localtime( time + $time_remaining );
	return (
		sprintf(
			"ETA: %s  %1.0f s  %s/%s msgs left",
...
sub tests_time_remaining {
	...
	ok( 1 == time_remaining( 9, 9, 10, 9 ),
		"time_remaining: 9, 9, 10, 9 -> 1" );
	ok( 9 == time_remaining( 1, 1, 10, 1 ),
		"time_remaining: 1, 1, 10, 1 -> 1" );
	return ();
}
...
sub tests_cache_map {
	...
	my ( $c12, $c21 );
	ok( ( $c12, $c21 ) = cache_map( \@cache_files, $msgs_1, $msgs_2 ),
		'cache_map: 02' );
	my $a1 = [ sort { $a <=> $b } keys %$c12 ];
	my $a2 = [ sort { $a <=> $b } keys %$c21 ];
	ok( 0 == compare_lists( [ 130, 142, 143, 177, 182, 1601 ], $a1 ),
		'cache_map: 03' );
	ok( 0 == compare_lists( [ 242, 243, 260, 279, 280, 2301 ], $a2 ),
		'cache_map: 04' );
...
sub tests_cache_dir_fix {
	ok( 'lalala' eq cache_dir_fix('lalala'),
		'cache_dir_fix: lalala -> lalala' );
	ok(
		'ii\\\\ii' eq cache_dir_fix('ii\ii'),
		'cache_dir_fix: ii\ii -> ii\\\\ii'
	);
	ok( 'ii@ii' eq cache_dir_fix('ii@ii'), 'cache_dir_fix: ii@ii -> ii@ii' );
	ok(
		'ii@ii\\:ii' eq cache_dir_fix('ii@ii:ii'),
		'cache_dir_fix: ii@ii:ii -> ii@ii\\:ii'
	);
	ok(
		'i\\\\i\\\\ii' eq cache_dir_fix('i\i\ii'),
		'cache_dir_fix: i\i\ii -> i\\\\i\\\\ii'
	);
	ok(
		'i\\\\ii' eq cache_dir_fix('i\\ii'),
		'cache_dir_fix: i\\ii -> i\\\\\\\\ii'
	);
	ok( '\\\\ ' eq cache_dir_fix('\\ '), 'cache_dir_fix: \\  -> \\\\\ ' );
	ok( '\\\\ ' eq cache_dir_fix('\ '),  'cache_dir_fix: \  -> \\\\\ ' );
	ok(
		'\[bracket\]' eq cache_dir_fix('[bracket]'),
		'cache_dir_fix: [bracket] -> \[bracket\]'
	);
	return ();
}
...
sub tests_cache_dir_fix_win {
	ok(
		'lalala' eq cache_dir_fix_win('lalala'),
		'cache_dir_fix_win: lalala -> lalala'
	);
	ok(
		'[[]bracket[]]' eq cache_dir_fix_win('[bracket]'),
		'cache_dir_fix_win: [bracket] -> [[]bracket[]]'
	);
	return ();
}

sub get_cache {
	my ( $cache_dir, $h1_msgs_ref, $h2_msgs_ref, $h1_msgs_all_hash_ref,
		$h2_msgs_all_hash_ref )
	  = @_;
...
sub tests_get_cache {
	...
	ok( ( not -d 'W/tmp/cache/F1/F2' or rmtree('W/tmp/cache/F1/F2') ),
		'get_cache: rmtree W/tmp/cache/F1/F2' );
	...
	my $msgs_all_2 =
	  { 242 => '', 243 => '', 299 => '', 377 => '', 777 => '', 255 => '' };

	my ( $c12, $c21 );
	ok(
		( $c12, $c21 ) = get_cache(
			'W/tmp/cache/F1/F2', $msgs_1, $msgs_2, $msgs_all_1, $msgs_all_2
		),
		'get_cache: 02'
	);
	my $a1 = [ sort { $a <=> $b } keys %$c12 ];
	...
	ok( -f 'W/tmp/cache/F1/F2/142_242', 'get_cache: file kept 142_242' );
	ok( -f 'W/tmp/cache/F1/F2/142_242', 'get_cache: file kept 143_243' );
	ok( !-f 'W/tmp/cache/F1/F2/100_200',
		'get_cache: file NOT removed 100_200' );
	ok( !-f 'W/tmp/cache/F1/F2/101_201',
		'get_cache: file NOT removed 101_201' );

	# strange files
	#$debugcache = 1 ;
	$maxage = undef;
	ok( ( not -d 'W/tmp/cache/rr\uee' or rmtree('W/tmp/cache/rr\uee') ),
		'get_cache: rmtree W/tmp/cache/rr\uee' );
	...
	ok(
		( $c12, $c21 ) = get_cache(
			'W/tmp/cache/rr\uee', $msgs_1, $msgs_2,
			$msgs_all_1,          $msgs_all_2
		),
		'get_cache: strange path 02'
	);
	$a1 = [ sort { $a <=> $b } keys %$c12 ];
	$a2 = [ sort { $a <=> $b } keys %$c21 ];
	ok( 0 == compare_lists( [ 142, 143, 177 ], $a1 ),
		'get_cache: strange path 03' );
	ok( 0 == compare_lists( [ 242, 243, 777 ], $a2 ),
		'get_cache: strange path 04' );
	ok( -f 'W/tmp/cache/rr\uee/142_242',
		'get_cache: strange path file kept 142_242' );
	ok( -f 'W/tmp/cache/rr\uee/142_242',
		'get_cache: strange path file kept 143_243' );
	ok( !-f 'W/tmp/cache/rr\uee/100_200',
		'get_cache: strange path file removed 100_200' );
	ok( !-f 'W/tmp/cache/rr\uee/101_201',
		'get_cache: strange path file removed 101_201' );
	return ();
}
...
sub tests_clean_cache {
	...
	ok( ( not -d 'W/tmp/cache/G1/G2' or rmtree('W/tmp/cache/G1/G2') ),
		'clean_cache: rmtree W/tmp/cache/G1/G2' );
	...
	ok( clean_cache( \@test_files_cache, $cache, $all_1, $all_2 ),
		'clean_cache: ' );
...
sub tests_clean_cache_2 {
	...
	ok( ( not -d 'W/tmp/cache/G1/G2' or rmtree('W/tmp/cache/G1/G2') ),
		'clean_cache_2: rmtree W/tmp/cache/G1/G2' );
	ok( mkpath('W/tmp/cache/G1/G2'),
		'clean_cache_2: mkpath W/tmp/cache/G1/G2' );
	...
	ok( touch(@test_files_cache),
		'clean_cache_2: touch W/tmp/cache/G1/G2/...' );
...
sub tests_mkpath {
	...
	( 'MSWin32' ne $OSNAME ) and ok(
		(
			-d "W/tmp/tests/long/$long_path_unix"
			  or mkpath("W/tmp/tests/long/$long_path_unix")
		),
		'tests_mkpath: mkpath > 300 char'
	);
	( 'MSWin32' ne $OSNAME ) and ok(
		(
			-d "W/tmp/tests/long/$long_path_unix"
			  and rmtree("W/tmp/tests/long/")
		),
		'tests_mkpath: rmtree > 300 char'
	);
	...
	( 'MSWin32' eq $OSNAME )
	  and ok( ( -d $long_path_2_prefix and rmtree($long_path_2_prefix) ),
		'tests_mkpath: rmtree \ > 300 char' );

	( 'MSWin32' eq $OSNAME )
	  and ok( ( -d $long_path_2_prefix or mkpath("\\\\\?\\E:\\\\TEMP") ),
		'tests_mkpath: -d \ small path 1' );
	( 'MSWin32' eq $OSNAME )
	  and ok( ( -d $long_path_2 ), 'tests_mkpath: -d \ > 300 char' );
	( 'MSWin32' eq $OSNAME )
	  and ok( ( -d $long_path_2_prefix or mkpath($long_path_2_prefix) ),
		'tests_mkpath: -d \ small path' );
	( 'MSWin32' eq $OSNAME ) and ok(
		( -d $long_path_2 or mkpath($long_path_2) ),
		'tests_mkpath: mkpath \ > 300 char'
	);
	( 'MSWin32' eq $OSNAME )
	  and ok( ( -d $long_path_2_prefix and rmtree($long_path_2_prefix) ),
...
sub tests_touch {
	...
	ok( ( -d 'W/tmp/tests/' or mkpath('W/tmp/tests/') ),
		'tests_touch: mkpath W/tmp/tests/' );
	...
	ok( 1 == touch( 'W/tmp/tests/lili', 'W/tmp/tests/lolo' ),
		'tests_touch: 2 files' );
	ok(
		0 == touch( 'W/tmp/tests/\y', '/aaa' ),
		'tests_touch: 2 files, 1 fails'
	);
	return ();
}
...
sub tests_cache_folder {
	...
	ok(
		'/path/fold1/fold2' eq cache_folder( '', '/path', 'fold1', 'fold2' ),
		'cache_folder: /path, fold1, fold2 -> /path/fold1/fold2'
	);
	ok(
		'/pa_th/fold1/fold2' eq cache_folder( '', '/pa*th', 'fold1', 'fold2' ),
		'cache_folder: /pa*th, fold1, fold2 -> /path/fold1/fold2'
	);
	ok(
		'/_p_a__th/fol_d1/fold2' eq
		  cache_folder( '', '/>p<a|*th', 'fol*d1', 'fold2' ),
		'cache_folder: />p<a|*th, fol*d1, fold2 -> /path/fol_d1/fold2'
	);

	ok(
		'D:/path/fold1/fold2' eq
		  cache_folder( 'D:', '/path', 'fold1', 'fold2' ),
		'cache_folder: /path, fold1, fold2 -> /path/fold1/fold2'
	);
	ok(
		'D:/pa_th/fold1/fold2' eq
		  cache_folder( 'D:', '/pa*th', 'fold1', 'fold2' ),
		'cache_folder: /pa*th, fold1, fold2 -> /path/fold1/fold2'
	);
	ok(
		'D:/_p_a__th/fol_d1/fold2' eq
		  cache_folder( 'D:', '/>p<a|*th', 'fol*d1', 'fold2' ),
		'cache_folder: />p<a|*th, fol*d1, fold2 -> /path/fol_d1/fold2'
	);
	ok( '//' eq cache_folder( '', '', '', '' ), 'cache_folder:  -> //' );
	ok( '//_______' eq cache_folder( '', '', '', '*|?:"<>' ),
		'cache_folder: *|?:"<> -> //_______' );
	return ();
}
...
sub tests_filter_forbidden_characters {
	...
	ok(
		'a_b' eq filter_forbidden_characters('a_b'),
		'filter_forbidden_characters: a_b -> a_b'
	);
	ok(
		'a_b' eq filter_forbidden_characters('a*b'),
		'filter_forbidden_characters: a*b -> a_b'
	);
	ok(
		'a_b' eq filter_forbidden_characters('a|b'),
		'filter_forbidden_characters: a|b -> a_b'
	...
	ok(
		'a_______b' eq filter_forbidden_characters('a*|?:"<>b'),
		'filter_forbidden_characters: a*|?:"<>b -> a_______b'
	);

	( 'MSWin32' ne $OSNAME ) and ok(
		( 'a b ' eq filter_forbidden_characters('a b ') ),
		'filter_forbidden_characters: "a b " -> "a b "'
	);

	( 'MSWin32' eq $OSNAME ) and ok(
		( ' a b_' eq filter_forbidden_characters(' a b ') ),
		'filter_forbidden_characters: "a b " -> "a b_"'
	);
	( 'MSWin32' eq $OSNAME ) and ok(
		( ' a b_/ c d_' eq filter_forbidden_characters(' a b / c d ') ),
		'filter_forbidden_characters: " a b / c d " -> "a b_/ c d_"'
	);
...
sub tests_convert_sep_to_slash {
	...
	ok(
		'' eq convert_sep_to_slash( '', '/' ),
		'convert_sep_to_slash: no folder'
	);
	ok( 'INBOX' eq convert_sep_to_slash( 'INBOX', '/' ),
		'convert_sep_to_slash: INBOX' );
	ok( 'INBOX/foo' eq convert_sep_to_slash( 'INBOX/foo', '/' ),
		'convert_sep_to_slash: INBOX/foo' );
	ok( 'INBOX/foo' eq convert_sep_to_slash( 'INBOX_foo', '_' ),
		'convert_sep_to_slash: INBOX_foo' );
	ok( 'INBOX/foo/zob' eq convert_sep_to_slash( 'INBOX_foo_zob', '_' ),
		'convert_sep_to_slash: INBOX_foo_zob' );
	ok( 'INBOX/foo' eq convert_sep_to_slash( 'INBOX.foo', '.' ),
		'convert_sep_to_slash: INBOX.foo' );
	ok( 'INBOX/foo/hi' eq convert_sep_to_slash( 'INBOX.foo.hi', '.' ),
		'convert_sep_to_slash: INBOX.foo.hi' );
	return ();
}
...
sub tests_regexmess {
	...
	ok(
		...
		"regexmess, no regexmess, nothing to do"
	);
	...
	ok( '' eq regexmess(''), 'From mbox 1 add colon blank' );

	ok(
		'From:<tartanpion@machin.truc>' eq
		  regexmess('From <tartanpion@machin.truc>'),
	...
	@regexmess = 's{\AFrom\ [^\n]*(\n)?}{}gxms';
	ok( '' eq regexmess(''), 'From mbox 1 remove, blank' );

	ok( '' eq regexmess('From <tartanpion@machin.truc>'),
		'From mbox 2 remove' );
...
EOM
	...
	@regexmess =
	  's{\A(.*?(?! ^$))(^Disposition-Notification-To:.*?\n)}{$1}gxms';
...
sub tests_bytes_display_string {
	...
	ok(
		'0.000 KiB' eq bytes_display_string(0),
		'bytes_display_string:       0'
	);
	ok(
		'0.001 KiB' eq bytes_display_string(1),
		'bytes_display_string:       1'
	);
	ok(
		'0.010 KiB' eq bytes_display_string(10),
		'bytes_display_string:      10'
	);
	ok( '1.000 MiB' eq bytes_display_string(1048575),
		'bytes_display_string: 1048575' );
	ok( '1.000 MiB' eq bytes_display_string(1048576),
		'bytes_display_string: 1048576' );

	ok( '1.000 GiB' eq bytes_display_string(1073741823),
		'bytes_display_string: 1073741823 ' );
	ok( '1.000 GiB' eq bytes_display_string(1073741824),
		'bytes_display_string: 1073741824 ' );

	ok( '1.000 TiB' eq bytes_display_string(1099511627775),
		'bytes_display_string: 1099511627775' );
	ok( '1.000 TiB' eq bytes_display_string(1099511627776),
		'bytes_display_string: 1099511627776' );

	ok( '1.000 PiB' eq bytes_display_string(1125899906842623),
		'bytes_display_string: 1125899906842623' );
	ok( '1.000 PiB' eq bytes_display_string(1125899906842624),
		'bytes_display_string: 1125899906842624' );

	ok( '1024.000 PiB' eq bytes_display_string(1152921504606846975),
		'bytes_display_string: 1152921504606846975' );
	ok( '1024.000 PiB' eq bytes_display_string(1152921504606846976),
		'bytes_display_string: 1152921504606846976' );
...
sub bytes_display_string {
  ...
  SWITCH: {
		...
		if ( abs($bytes) < ( 1000 * 1024 * 1024 * 1024 ) ) {
			$readable_value =
			  sprintf( "%.3f GiB", $bytes / ( 1024 * 1024 * 1024 ) );
			last SWITCH;
		}
		if ( abs($bytes) < ( 1000 * 1024 * 1024 * 1024 * 1024 ) ) {
			$readable_value =
			  sprintf( "%.3f TiB", $bytes / ( 1024 * 1024 * 1024 * 1024 ) );
			last SWITCH;
		}
		else {
			$readable_value = sprintf( "%.3f PiB",
...
sub stats {
	...
	my $memory_ratio =
	  ($max_msg_size_in_bytes)
	  ? sprintf( '%.1f', $memory_consumption / $max_msg_size_in_bytes )
	  : "NA";
...
sub header_line_normalize {
	...
	if ( ($messageidnodomain) and ( 'MESSAGE-ID' eq uc($header_key) ) ) {
		$header_val =~ s/^([^@]+).*$/$1/xo;
	}
...
sub tests_header_line_normalize {
	...
	ok(
		': ' eq header_line_normalize( '', '' ),
		'header_line_normalize: empty args'
	);
	ok( 'HHH: VVV' eq header_line_normalize( 'hhh', 'vvv' ),
		'header_line_normalize: hhh vvv ' );
	ok(
		'HHH: VVV' eq header_line_normalize( 'hhh', '  vvv' ),
		'header_line_normalize: remove first blancs'
	);
	ok( 'HHH: AA BB CCC ' eq header_line_normalize( 'hhh', 'aa  bb   ccc  ' ),
		'header_line_normalize: remove succesive blancs' );
	ok( 'HHH: VVV XX YY' eq header_line_normalize( 'hhh', "vvv\t\txx\tyy" ),
		'header_line_normalize: tabs' );
	ok( 'HHH: XABX' eq header_line_normalize( 'hhh', "\x80AB\xff" ),
		'header_line_normalize: 8bit' );
...
sub tests_is_a_release_number {
	...
	ok(
		is_a_release_number( imapsync_version() ),
		'is_a_release_number imapsync_version()'
	);
...
m{(?:.{3})\s(...)\s+(\d{1,2})\s(\d{1,2}):(\d{1,2}):(\d{1,2})\s(?:\w{3})?\s?(\d{4})}xo
		...
		if ( $d =~ m{(\d{2})/(\d{2})/(\d{2})\s(\d{2}):(\d{2}):(\d{2})}xo ) {
			...
			$year = '20' . $year;
			my %num2mon =
			  qw(01 Jan 02 Feb 03 Mar 04 Apr 05 May 06 Jun 07 Jul 08 Aug 09 Sep 10 Oct 11 Nov 12 Dec);
			$month = $num2mon{$month};
			$d     = "$day-$month-$year $hour:$min:$sec +0000";
			last SWITCH;
		}
...
m{\w{6,},\s(\w{3})\w+\s+(\d{1,2}),\s(\d{4})\s(\d{2}):(\d{2})\s(AM|PM)}xo
		  )
		{
		 ...
		 # Saturday, December 14, 2002 05:00 PM - KBtoys.com order confirmations
			...
			my ( $month, $day, $year, $hour, $min, $apm ) =
			  ( $1, $2, $3, $4, $5, $6 );
...
m{(\w{3})\s(\d{1,2})\s(\d{4})\s(\d{2}):(\d{2}):(\d{2})\s((?:\+|-)\d{4})}xo
		  )
		{
			...
			my ( $month, $day, $year, $hour, $min, $sec, $zone ) =
			  ( $1, $2, $3, $4, $5, $6, $7 );
...
sub tests_good_date {
	...
	ok(
		'"24-Aug-2010 16:00:00 +0200"' eq
		  good_date('24-Aug-2010 16:00:00 +0200'),
		'good_date internal 2digit zone'
	);
	ok( '"24-Aug-2010 16:00:00 +0000"' eq good_date('24-Aug-2010 16:00:00'),
		'good_date internal 2digit no zone' );
	ok(
		'"01-Sep-2010 16:00:00 +0200"' eq
		  good_date('1-Sep-2010 16:00:00 +0200'),
		'good_date internal SP 1digit'
	);
	ok(
		'"24-Aug-2010 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 2010 16:00:00 +0200'),
		'good_date header 2digit zone'
	);
	ok(
		'"01-Sep-2010 16:00:00 +0000"' eq good_date('Wed, 1 Sep 2010 16:00:00'),
		'good_date header SP 1digit zone'
	);
	ok(
		'"01-Sep-2010 16:00:00 +0200"' eq
		  good_date('Wed, 1 Sep 2010 16:00:00 +0200'),
		'good_date header SP 1digit zone'
	);
	ok(
		'"01-Sep-2010 16:00:00 +0200"' eq
		  good_date('Wed, 1 Sep 2010 16:00:00 +0200 (CEST)'),
		'good_date header SP 1digit zone'
	);
	ok( '"06-Feb-2009 22:18:08 +0000"' eq good_date('02/06/09 22:18:08'),
		'good_date header TemPageR' );
	ok(
		'"02-Jan-2008 08:40:57 +0000"' eq good_date('Wed Jan  2 08:40:57 2008'),
		'good_date header dice.com support 1digit day'
	);
	ok(
		'"20-Aug-2006 11:55:09 +0000"' eq good_date('Sun Aug 20 11:55:09 2006'),
		'good_date header dice.com support 2digit day'
	);
	ok(
		'"24-Jan-2007 11:58:38 +0000"' eq
		  good_date('Wed Jan 24 11:58:38 MST 2007'),
		'good_date header status-now.com'
	);
	ok(
		'"24-Aug-2010 16:00:00 +0200"' eq
		  good_date('24 Aug 2010 16:00:00 +0200'),
		'good_date header missing date of week'
	);
	ok(
		'"24-Aug-2067 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 67 16:00:00 +0200'),
		'good_date header 2digit year'
	);
	ok(
		'"24-Aug-1977 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 77 16:00:00 +0200'),
		'good_date header 2digit year'
	);
	ok(
		'"24-Aug-1987 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 87 16:00:00 +0200'),
		'good_date header 2digit year'
	);
	ok(
		'"24-Aug-1997 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 97 16:00:00 +0200'),
		'good_date header 2digit year'
	);
	ok(
		'"24-Aug-2004 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 04 16:00:00 +0200'),
		'good_date header 2digit year'
	);
	ok(
		'"24-Aug-1997 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 1997 16.00.00 +0200'),
		'good_date header period time sep'
	);
	ok(
		'"24-Aug-1997 16:00:00 +0200"' eq
		  good_date('Tue, 24 Aug 1997  16:00:00 +0200'),
		'good_date header extra white space type1'
	);
	ok(
		'"24-Aug-1997 05:06:02 +0200"' eq
		  good_date('Tue, 24 Aug 1997 5:6:2 +0200'),
		'good_date header 1digit time vals'
	);
	ok(
		'"24-Aug-1997 05:06:02 +0200"' eq
		  good_date('Tue, 24, Aug 1997 05:06:02 +0200'),
		'good_date header extra commas'
	);
	ok(
		'"01-Oct-2003 12:45:24 +0000"' eq
		  good_date('Wednesday, 01 October 2003 12:45:24 CDT'),
		'good_date header no abbrev'
	);
	ok(
		'"11-Jan-2005 17:58:27 -0500"' eq
		  good_date('Tue,  11  Jan 2005 17:58:27 -0500'),
		'good_date extra white space'
	);
	ok(
		'"18-Dec-2002 15:07:00 +0000"' eq
		  good_date('Wednesday, December 18, 2002 03:07 PM'),
		'good_date kbtoys.com orders'
	);
	ok(
		'"16-Dec-2004 02:01:49 -0500"' eq
		  good_date('Dec 16 2004 02:01:49 -0500'),
		'good_date jr.com orders'
	);
	ok( '"21-Jun-2001 11:11:11 +0000"' eq good_date('21-Jun-2001'),
		'good_date register.com domain transfer' );
	ok(
		'"18-Nov-2012 18:34:38 +0100"' eq
		  good_date('Sun, 18 Nov 2012 18:34:38 +0100'),
		'good_date pop2imap bug (Westeuropäische Normalzeit)'
	);
...
sub tests_list_keys_in_2_not_in_1 {
	...
	ok( 0 == compare_lists( [], [ list_keys_in_2_not_in_1( {}, {} ) ] ),
		'list_keys_in_2_not_in_1: {} {}' );
	ok(
		0 == compare_lists(
			[ 'a', 'b' ],
			[ list_keys_in_2_not_in_1( {}, { 'a' => 1, 'b' => 1 } ) ]
		),
		'list_keys_in_2_not_in_1: {} {a, b}'
	);
	ok(
		0 == compare_lists(
			['b'],
			[ list_keys_in_2_not_in_1( { 'a' => 1 }, { 'a' => 1, 'b' => 1 } ) ]
		),
		'list_keys_in_2_not_in_1: {a} {a, b}'
	);
	ok(
		0 == compare_lists(
			[],
		...
		'list_keys_in_2_not_in_1: {a, b} {a, b}'
	);
	ok(
		0 == compare_lists(
			[],
		...
		'list_keys_in_2_not_in_1: {a, b, c} {a, b}'
	);
	ok(
		0 == compare_lists(
			['b'],
...
sub list_folders_in_2_not_in_1 {
	...
	@h2_folders_not_in_h1 =
	  list_keys_in_2_not_in_1( \%h1_folders_all, \%h2_folders_all );
	map { $h2_folders_not_in_h1{$_} = 1 } @h2_folders_not_in_h1;
	@h2_folders_not_in_h1 = list_keys_in_2_not_in_1( \%h2_folders_from_1_all,
		\%h2_folders_not_in_h1 );
...
EOH
	...
	ok( 'VAL_3' eq $header_dec->{'KEY_3'}[0], 'decompose_header: VAL_3' );

	ok( 'VAL_1' eq $header_dec->{'KEY_1'}[0], 'decompose_header: VAL_1' );

	ok( 'VAL_1_other' eq $header_dec->{'KEY_1'}[1],
		'decompose_header: VAL_1_other' );

	ok(
		'VAL_2 VAL_2_+ VAL_2_++' eq $header_dec->{'KEY_2'}[0],
		'decompose_header: VAL_2 VAL_2_+ VAL_2_++'
	);

	ok( 'VAL_4 VAL_4_+' eq $header_dec->{'KEY_4'}[0],
		'decompose_header: VAL_4 VAL_4_+' );

	ok( ' VAL_5' eq $header_dec->{'KEY_5 BLANC'}[0],
		'decompose_header: KEY_5 BLANC' );

	ok( not( defined( $header_dec->{'KEY_6_BAD_BODY'}[0] ) ),
		'decompose_header: KEY_6_BAD_BODY' );
...
EOH
	...
	ok(
		'<20100428101817.A66CB162474E@plume.est.belle>' eq
		  $header_dec->{'Message-Id'}[0],
		'decompose_header: 1'
	);
...
EOH
	...
	ok(
		'VAL_3' eq $header_dec->{'KEY_3'}[0],
		'decompose_header: Bad header VAL_3'
	);

	ok(
		'VAL_1_other' eq $header_dec->{'KEY_1'}[0],
		'decompose_header: Bad header VAL_1_other'
	);

	ok(
		'VAL_2 VAL_2_+ VAL_2_++' eq $header_dec->{'KEY_2'}[0],
		'decompose_header: Bad header VAL_2 VAL_2_+ VAL_2_++'
	);
...
sub tests_epoch {
	ok(
		'1282658400' eq epoch('24-Aug-2010 16:00:00 +0200'),
		'epoch 24-Aug-2010 16:00:00 +0200 -> 1282658400'
	);
	ok(
		'1282658400' eq epoch('24-Aug-2010 14:00:00 +0000'),
		'epoch 24-Aug-2010 14:00:00 +0000 -> 1282658400'
	);
	ok(
		'1282658400' eq epoch('24-Aug-2010 12:00:00 -0200'),
		'epoch 24-Aug-2010 12:00:00 -0200 -> 1282658400'
	);
	ok(
		'1282658400' eq epoch('24-Aug-2010 16:01:00 +0201'),
		'epoch 24-Aug-2010 16:01:00 +0201 -> 1282658400'
	);
	ok(
		'1282658400' eq epoch('24-Aug-2010 14:01:00 +0001'),
		'epoch 24-Aug-2010 14:01:00 +0001 -> 1282658400'
	);

	ok(
		'1280671200' eq epoch('1-Aug-2010 16:00:00 +0200'),
		'epoch 1-Aug-2010 16:00:00 +0200 -> 1280671200'
	);
	ok(
		'1280671200' eq epoch('1-Aug-2010 14:00:00 +0000'),
		'epoch 1-Aug-2010 14:00:00 +0000 -> 1280671200'
	);
	ok(
		'1280671200' eq epoch('1-Aug-2010 12:00:00 -0200'),
		'epoch 1-Aug-2010 12:00:00 -0200 -> 1280671200'
	);
	ok(
		'1280671200' eq epoch('1-Aug-2010 16:01:00 +0201'),
		'epoch 1-Aug-2010 16:01:00 +0201 -> 1280671200'
	);
	ok(
		'1280671200' eq epoch('1-Aug-2010 14:01:00 +0001'),
		'epoch 1-Aug-2010 14:01:00 +0001 -> 1280671200'
	);
	return ();
}
...
sub tests_add_header {
	...
	ok( 'Message-Id: <123456789@imapsync>' eq add_header(123456789),
		'add_header 123456789' );
...
sub tests_logfile {
	ok(
		'LOG_imapsync/1970_01_01_01_00_00.txt' eq logfile(),
		'logfile: no args => LOG_imapsync/1970_01_01_01_00_00.txt'
	);
	ok(
		'LOG_imapsync/1970_01_01_01_00_00.txt' eq logfile(0),
		'logfile: 0 => LOG_imapsync/1970_01_01_01_00_00.txt'
	);
	ok(
		'LOG_imapsync/1970_01_01_01_01_01.txt' eq logfile(61),
		'logfile: 0 => LOG_imapsync/1970_01_01_01_01_01.txt'
	);
	ok(
		'LOG_imapsync/2010_08_24_16_00_00.txt' eq logfile(1282658400),
		'logfile: 1282658400 => LOG_imapsync/2010_08_24_16_00_00.txt'
	);
	ok(
		'LOG_imapsync/2010_08_24_16_01_01.txt' eq logfile(1282658461),
		'logfile: 1282658461 => LOG_imapsync/2010_08_24_16_01_01.txt'
	);
	ok(
		'LOG_imapsync/2010_08_24_16_01_01_poupinette.txt' eq
		  logfile( 1282658461, "poupinette" ),
'logfile: 1282658461 poupinette => LOG_imapsync/2010_08_24_16_01_01_poupinette.txt'
	);
...
sub teelaunch {
	my $logfile = shift;
	logfileprepa($logfile)
	  || croak "Error creating directory for $logfile : $!";
	my $logfile_handle;
	open( $logfile_handle, '>', $logfile )
	  or die("Can not open $logfile for write: $!");
...
sub Reconnect_counter {
	...
	$self->{Reconnect_counter} = 0
	  if ( not defined( $self->{Reconnect_counter} ) );
